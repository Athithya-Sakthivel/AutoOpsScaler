...
ssh "$NAME" "mkdir -p ~/.pulumi/plugins/resource-${PLUGIN_NAME}-v${PLUGIN_VERSION}"
scp "$PLUGIN_DIR/pulumi-resource-${PLUGIN_NAME}" "$NAME:~/.pulumi/plugins/resource-${PLUGIN_NAME}-v${PLUGIN_VERSION}/"
echo "✔️ Pulumi plugin injected inside VM."

# ─────────────────────────────────────────────
# STEP 6: Open VS Code with Remote SSH
