...
├── Makefile                             # LowOps commands for devs like make create-local-cluster, make create-prod-cluster, make deploy-indexing-pipeline, etc.
...
|
├── .devcontainer/
│   ├── devcontainer.json                # VS Code dev container config for local development with Python, Ray, and other tools.
│   └── Dockerfile                       # Builds dev container with Python, Ray, and all necessary tools for local development.
|
├── dev/
...
│   ├── ray_job.yaml                     # RayJob spec to run extract + upload logic asynchronously on Ray cluster.
│   ├── app-extract-load.argocd.yaml     # Argo CD Application manifest for GitOps sync of extract-load pipeline.  ← renamed & updated annotation
│   └── modules/
...
│       ├── scraper.py                   # Web scraper using Scrapy + Playwright; should log errors and emit trace spans due to failure-prone ops.
│       ├── s3_uploader.py               # Uploads raw docs to S3 with boto3; latency metrics via Prometheus can help detect bottlenecks.
│       ├── main.py                      # Orchestrates entire extract pipeline; wrap stages with Langfuse spans for visibility.
...
│   ├── ray_job.yaml                     # RayJob manifest to execute distributed parsing and chunking workflows.
│   ├── app-data-preprocess.argocd.yaml  # Argo CD Application manifest for GitOps sync of data preprocessing pipeline.  ← renamed & updated annotation
│   └── modules/
...
│       ├── main.py                      # Orchestrates full preprocessing pipeline; wrap with Langfuse/OpenLLMetry for full traceability.
...
│   ├── ray_job.yaml                     # RayJob manifest to run embedding batch jobs on KubeRay.
│   ├── app-embedding.argocd.yaml        # Argo CD Application manifest for GitOps sync of embedding pipeline.  ← renamed & updated annotation
│   └── modules/
│       ├── __init__.py                  # Marks modules as a Python package.
│       ├── batch_embed.py               # Orchestrates batch embedding flow; should include Prometheus metrics.
...
│   ├── ray_job.yaml                     # RayJob to push embeddings into Qdrant DB via batch ingestion.
│   ├── app-vector.argocd.yaml           # Argo CD Application manifest for GitOps sync of Qdrant vector ingestion.  ← renamed & updated annotation
│   └── modules/
│       ├── __init__.py                  # Marks modules as a Python package.
...
│   ├── ray_job.yaml                     # RayJob to interact with metadata DB (Supabase).
│   ├── app-supabase.argocd.yaml         # Argo CD Application manifest for GitOps sync of Supabase metadata tasks.  ← renamed & updated annotation
│   └── modules/
│       ├── __init__.py                  # Marks modules as a Python package.
...
│   │   ├── ray_job.yaml                 # RayJob to launch RAG indexing or real-time retrieval.
│   │   ├── app-rag.argocd.yaml          # Argo CD Application manifest for GitOps sync of RAG orchestration.  ← renamed & updated annotation
│   │   └── modules/
│   │       ├── __init__.py              # Marks modules as a Python package.
│   │       ├── generator.py             # Calls LLM for response; must log Langfuse spans and token usage.
│   │       ├── pipeline.py              # End-to-end orchestration logic for RAG; should be traced and metered.
...
│   │   ├── ray_job.yaml                 # RayJob to run evaluations using RAGAS on inference outputs.
│   │   └── modules/
│   │       ├── __init__.py              # Marks modules as a Python package.
...
│   │   │   ├── ray_service.yaml         # Ray Serve config for deploying API as long-lived service.
│   │   │   ├── app-api.argo.yml         # Argo CD Application manifest for GitOps sync of backend API.  ← already correct
...
│   ├── ray_service.yaml                 # RayService to deploy this as a persistent microservice (not batch job); integrates monitoring.
│   ├── app-orchestrator.argocd.yaml     # Argo CD Application manifest for GitOps sync of orchestrator service.  ← renamed & updated annotation
│   └── modules/
...
│   ├── ray_service.yaml                 # Persistent RayService to collect system/LLM metrics and serve dashboards for observability.
│   └── modules/
...
├── infra/
│   ├── carvel/
│   │   ├── llmops-api/                  # Carvel package for exposing the orchestrator API or RAG inference as a Kubernetes service.
│   │   ├── qdrant/                      # Carvel package for deploying Qdrant vector DB cluster (GPU-optional) in k8s.
│   │   ├── ray/                         # Carvel package for Ray cluster with autoscaler, RayDashboard and Serve enabled for observability.
│   │   ├── supabase/                    # Carvel package for deploying Supabase for metadata store (with Postgres & Realtime).
│   │   └── kapp-values/
│   │       ├── dev.yaml                 # overrides for dev environment: CPU-only, low replicas, debug logging, etc.
│   │       └── prod.yaml                # Production overrides: GPU-enabled, HPA, tracing + Prometheus exporters enabled.
│   └── terraform/
│       ├── eks.tf                       # Creates EKS cluster with Karpenter for node autoscaler, node groups, IAM roles for S3/Qdrant access.
...
|
└── workflows/                           # Replaced .github CI with Argo Workflows for scalability and flexibility.
    ├── cd-argoworkflow.yaml             # Argo Workflow to apply Carvel packages and Terraform infra on push to main or release.
    ├── ci-argoworkflow.yaml             # Argo Workflow runs full lint/test/validation suite on PRs.
    ├── infra-check-argowf.yaml          # Validates Terraform fmt/plan and Carvel manifest lint for safety.
    ├── indexing-pipeline-argowf.yaml    # Argo Workflow to trigger extract + ingest + embed + vector insert pipeline.
    ├── inference-pipeline-argowf.yaml   # Argo Workflow for end-to-end RAG/inference deployment validation + test.
    └── docker-publish-argowf.yaml       # Builds and pushes module Docker images to GHCR or Docker Hub on tag push.
